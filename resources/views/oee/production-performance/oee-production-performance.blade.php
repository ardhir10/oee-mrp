                ...
                <div class="row">
                    ...
                    <div class="col-12"  >
                        ...
                        <table class="table table-bordered " id="machine-{{$oee_machine->name}}">
                            <thead>
                                <th colspan="4" style="font-size: 18px; cursor: pointer;" onclick="showModal('{{ $oee_machine->id }}')">
                                    Machine-<span style="font-size: 18px; color:red;">{{$oee_machine->ident}}</span>
                            ...
                            <tbody>
                                <tr>
                                    <td width="30%">
                                        <a href="{{route('oee.production-performance-product-out',$oee_machine->name)}}">
...
@php
    $date = date('Y-m-d');
@endphp
@foreach ($oee_machines as $item)
        <!-- Modal -->
        <div class="modal  fade" id="alarm-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content ">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLongTitle">{{ $item->ident }}</h5>
                ...
                <div class="modal-body">
                    @php
                        $totalAlarm = [];
                    @endphp
                    @foreach ($item->oeeAlarmMaster as $alarm)
                        @foreach ($alarm->alarmList as $list)
                            @if (date('Y-m-d', strtotime($list->created_at)) == $date)
                            @php
                                if (!isset($totalAlarm[$list->alarmMaster->alarm_name])) {
                                    $totalAlarm[$list->alarmMaster->alarm_name] =  1;
                                    # code...
                                }else{
                                    $totalAlarm[$list->alarmMaster->alarm_name] =  $totalAlarm[$list->alarmMaster->alarm_name]+1;
                                }
                            @endphp
                            @endif
                        @endforeach
                    @endforeach
                    @foreach ($totalAlarm as $key => $value)
                                <span class="badge badge-primary" style="font-size: 15px">{{ $key }} : {{ $value }}</span>
                            ...
                            @foreach ($item->oeeAlarmMaster as $alarm)
                                @foreach ($alarm->alarmList as $list)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$list->alarmMaster->machine->ident}}</td>
                                        <td>{{$list->alarmMaster->alarm_name}}</td>
                                        <td>{{$list->text}}</td>
                                    </tr>
                                    @endif
                                @endforeach
